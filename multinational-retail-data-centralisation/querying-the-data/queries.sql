...
SELECT locality, COUNT(store_code) AS total_no_stores FROM dim_store_details GROUP BY locality ORDER BY total_no_stores DESC, locality LIMIT 7;
-- maybe come back to this one - instead of using limit 7, is this result the 75th percentile?
...
SELECT ROUND(SUM(total_payment_taken)::numeric,2) AS total_sales, month
FROM cte GROUP BY month ORDER BY total_sales DESC LIMIT 6;
-- NB I got an error when I tried to round without numeric::2
...
-- product_quantity and store_code and date_uuid in the orders_table
...
SELECT store_type, total_sales_per_store AS total_sales,
...
-- my percentage values were slightly off the expected results shown on the portal
-- I manually checked the answers and the portal results were incorrect
-- total revenue: 7,722,333.64
-- manually checking my percentages - first two mine were correct and the answers provided were wrong
